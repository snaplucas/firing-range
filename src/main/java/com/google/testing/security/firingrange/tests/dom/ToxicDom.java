...
/**
 * A servlet to provide toxic DOM based XSSes. It is mapped to the whole /pages.dom/toxicdom/
 ...
 * </ul>
 *
 * Thus, {@code /pages.dom/toxicdom/document/cookie/eval} will map to the document.cookie source,
 * and the eval sink.
 */
public class ToxicDom extends HttpServlet {
  ...
  private String generateExternalTemplate(String source, String accessType, String sink)
    ...
    return Templates.replacePayload(
        toxicTemplate, "/pages/dom/toxicdomscripts/" + source + "/" + accessType + "/" + sink);
  }
}
