...
/**
 * A class only allowing a given set of pages.tags and properties of those pages.tags as its input payload.
 */
public class TagServlet extends HttpServlet {
  ...
  public void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException {
    ...
    if (elements.isEmpty()) {
      Responses.sendError(response, "Invalid input, no pages.tags", 400);
      return;
    }
  ...
  /**
   * Handles the request filtering out unallowed pages.tags. Note that an empty allowedTag we allow
   * all pages.tags.
   */
