...
/**
 * Handles XSS via pages.redirect provided via a GET parameter.
 */
public class Parameter extends HttpServlet {
  @VisibleForTesting
  public static final String ECHOED_PARAM = "url";
  ...
  public void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException {
    ...
    if (pathInfo.startsWith("/url/")) {
      // /pages.redirect/parameter/url/a/b/c?x=y  gets redirected to /a/b/c?x=y
      ...
      // /pages.redirect/parameter?url=......  gets redirected to the specified
  ...
  private void handleNoStartWithJs(String param, HttpServletResponse response) throws IOException {
    if (param.startsWith("javascript")) {
      Responses.sendError(response, "Cannot pages.redirect to javascript!", 400);
    } else {
