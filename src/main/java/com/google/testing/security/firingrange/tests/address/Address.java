...
/**
 * Handles pages.address based DOM XSS, through various sources and sinks.
 */
public class Address extends HttpServlet {
    private static final String BASE_TEMPLATE = "pages.address.tmpl";

    @Override
    public void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException {
        String pathInfo = Strings.isNullOrEmpty(request.getPathInfo()) ? "" : request.getPathInfo().substring(1);

        if (CharMatcher.is('/').countIn(pathInfo) != 1) {
            String errorMsg = String.format("Missing sink, not enough sinks or too many sinks :(." + "Got %d, expected 1", CharMatcher.is('/').countIn(pathInfo));
            ...
            try {
                ...
                if (pathInfoParts.length == 2) {
                    Responses.sendXssed(response, generateTemplate(pathInfoParts[0], pathInfoParts[1]));
                } else {
