package unit.tests.tests.reverseclickjacking;
...
import com.google.testing.security.firingrange.tests.reverseclickjacking.UniversalReverseClickjackingSinglePage;
...
public class UniversalReverseClickjackingSinglePageTest {
  ...
  public void returnsPageParameterInFragmentInCallback() throws IOException {
    ...
    new UniversalReverseClickjackingSinglePage().doGet(request, response);
    verify(response).setStatus(200);
    // Verify that we return the right template, with the parameter pages.reflected correctly
